...
2. **Reference Resolution**: To avoid breaking references, the Kustomize engine tracks these name changes. It updates any referencing fields—such as \`volumes.configMap.name\`, \`envFrom.configMapRef.name\`, or \`env.valueFrom.configMapKeyRef.name\`—within matching Workloads (Deployments, StatefulSets) using built-in field specs that match resource kinds.
