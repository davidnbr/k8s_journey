...
function ServiceBadge({
  ...
  const hoverRing = isHovered
    ? 'ring-2 ring-blue-400 ring-offset-1 ring-offset-slate-900 scale-[1.02]'
    : ''
      ...
      {matchedPods.length > 0 && (
        <div className="mt-1.5 pt-1.5 border-t border-slate-700/40 text-[9px] opacity-80 flex flex-wrap gap-1">
          ...
          {matchedPods.map((p) => (
            <span
              key={p.id}
              className="bg-slate-950/40 px-1 rounded border border-slate-700/20 text-slate-300 font-mono"
            >
    ...
    <div className="bg-slate-950 border border-slate-700 rounded-xl p-4 h-full flex flex-col gap-3 font-mono text-xs select-none">
      ...
      <div className="grid grid-cols-2 gap-2 flex-1">
        ...
        >
          ...
          {hl === 'kubelet' && <div className="text-[9px] text-green-400/70 mb-1">kubelet ↻</div>}
              ...
              return (
                ...
                >
                  <div className="font-bold flex items-center gap-1">
                    <span
                      className={`w-1.5 h-1.5 rounded-full ${ready ? 'bg-emerald-400' : 'bg-yellow-400 animate-pulse-dot'}`}
                    />
                    {dep.name}
                  </div>
                  <div className="text-[10px] opacity-70">
                    {dep.availableReplicas}/{dep.replicas} ready ·{' '}
